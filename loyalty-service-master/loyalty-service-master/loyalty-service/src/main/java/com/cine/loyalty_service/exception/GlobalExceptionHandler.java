package com.cine.loyalty_service.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> manejarValidacion(MethodArgumentNotValidException ex) {
        logger.warn("Error de validación en loyalty-service: {}", ex.getMessage());
    ...
    // CONFLICTO DE LÓGICA DE NEGOCIO (ej: cuenta ya existe) → 409 Conflict
    @ExceptionHandler(IllegalArgumentException.class)
    public ResponseEntity<Map<String, Object>> manejarConflicto(IllegalArgumentException ex) {
        logger.warn("Conflicto de lógica de negocio: {}", ex.getMessage());
    ...
    public ResponseEntity<Map<String, Object>> manejarRuntime(RuntimeException ex) {
        logger.error("Recurso no encontrado en loyalty-service: {}", ex.getMessage());
    ...
    public ResponseEntity<Map<String, Object>> manejarGeneral(Exception ex) {
        logger.error("Error inesperado en loyalty-service: {}", ex.getMessage(), ex);
